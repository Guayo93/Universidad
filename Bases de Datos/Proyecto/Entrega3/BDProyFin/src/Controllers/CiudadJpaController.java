...
public class CiudadJpaController implements Serializable {
    ...
    public void create(Ciudad ciudad) throws PreexistingEntityException, Exception {
        if (ciudad.getPropiedadCollection() == null) {
            ciudad.setPropiedadCollection(new ArrayList<Propiedad>());
        }
        ...
        try {
            ...
            for (Propiedad propiedadCollectionPropiedadToAttach : ciudad.getPropiedadCollection()) {
            ...
            ciudad.setPropiedadCollection(attachedPropiedadCollection);
            em.persist(ciudad);
            for (Propiedad propiedadCollectionPropiedad : ciudad.getPropiedadCollection()) {
                Ciudad oldCodCiudadOfPropiedadCollectionPropiedad = propiedadCollectionPropiedad.getCodCiudad();
                propiedadCollectionPropiedad.setCodCiudad(ciudad);
                propiedadCollectionPropiedad = em.merge(propiedadCollectionPropiedad);
                if (oldCodCiudadOfPropiedadCollectionPropiedad != null) {
                    oldCodCiudadOfPropiedadCollectionPropiedad.getPropiedadCollection().remove(propiedadCollectionPropiedad);
                    oldCodCiudadOfPropiedadCollectionPropiedad = em.merge(oldCodCiudadOfPropiedadCollectionPropiedad);
                }
            }
    ...
    public void edit(Ciudad ciudad) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Ciudad persistentCiudad = em.find(Ciudad.class, ciudad.getCodCiudad());
            Collection<Propiedad> propiedadCollectionOld = persistentCiudad.getPropiedadCollection();
            Collection<Propiedad> propiedadCollectionNew = ciudad.getPropiedadCollection();
            ...
            ciudad.setPropiedadCollection(propiedadCollectionNew);
            ...
            for (Propiedad propiedadCollectionOldPropiedad : propiedadCollectionOld) {
                if (!propiedadCollectionNew.contains(propiedadCollectionOldPropiedad)) {
                    propiedadCollectionOldPropiedad.setCodCiudad(null);
            ...
            for (Propiedad propiedadCollectionNewPropiedad : propiedadCollectionNew) {
                if (!propiedadCollectionOld.contains(propiedadCollectionNewPropiedad)) {
                    Ciudad oldCodCiudadOfPropiedadCollectionNewPropiedad = propiedadCollectionNewPropiedad.getCodCiudad();
                    propiedadCollectionNewPropiedad.setCodCiudad(ciudad);
                    propiedadCollectionNewPropiedad = em.merge(propiedadCollectionNewPropiedad);
                    if (oldCodCiudadOfPropiedadCollectionNewPropiedad != null && !oldCodCiudadOfPropiedadCollectionNewPropiedad.equals(ciudad)) {
                        oldCodCiudadOfPropiedadCollectionNewPropiedad.getPropiedadCollection().remove(propiedadCollectionNewPropiedad);
                        oldCodCiudadOfPropiedadCollectionNewPropiedad = em.merge(oldCodCiudadOfPropiedadCollectionNewPropiedad);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Short id) throws NonexistentEntityException {
        ...
        try {
            ...
            Collection<Propiedad> propiedadCollection = ciudad.getPropiedadCollection();
            for (Propiedad propiedadCollectionPropiedad : propiedadCollection) {
                propiedadCollectionPropiedad.setCodCiudad(null);
                propiedadCollectionPropiedad = em.merge(propiedadCollectionPropiedad);
            }
            em.remove(ciudad);
            em.getTransaction().commit();
    ...
    public Ciudad findCiudad(Short id) {
        EntityManager em = getEntityManager();
        try {
